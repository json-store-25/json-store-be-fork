...
## 정신우
> 도메인 분석에 직전 프로젝트 문서화가 미흡하여 하는데 어려움을 겪었습니다. "내가 아는 것을 전달하는 것"과 "모르는 것을 이해하는 것"을 위하여 문서화의 중요함을 느겼습니다.
이미 설계된 스키마에 새로운 식별자를 추가하는 작업을 수행하느라 고전을 겪었습니다. 어쩔수 없이 서비스 레이어, 프레젠테이션 레이어까지 변경점이 발생하여 수정에 어려움을 겪었습니다. 이로인해 초기 설계의 중요함을 다시금 깨닿게 되었습니다.
부하테스트 툴로 단순히 여러번 돌리는 것이 중요한게 정량적 성능 지표를 확실하게 얻는 방법은 타임스탬프 로그를 남기는 것이라는 것을 알게되었습니다.
부하테스트 도구로 테스트를 하며 시행착오를 겪었지만 그 자체로도, 스케일링 지표로 사용할 수 있었습니다.  성능 측정, 테스트를 본격적으로 다루어 본 경험이 없어 이번 프로젝트에서 좋은 이정표를 발견한 느낌입니다. 
굳이 카프카를 사용함에 있어 어려움이 있었습니다. 다른 메시징 플랫폼 혹은 redis로도 주문 순서 문제를 처리 할 수 있었기에 상황에 적절한 기술 적용의 중요함을 느꼈습니다.
마지막으로 제가 담당한 부분 외에 팀원 분들이 수행하신 부분을 잘 설명해주어 ELK 로깅, 프로메테우스, 그라파나에 대한 식견을 얻을 수 있었습니다.
